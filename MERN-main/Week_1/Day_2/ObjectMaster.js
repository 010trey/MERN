...
const pkmnNamesOver99 = pokémon.filter(p => p.id >99).map( p => p.name )

const poisonType = pokémon.filter(p => (p.types[0] == "poison")&&(p.types.length==1)).map( p => p.name )

const flyingPkmnTypes = pokémon.filter(p => p.types[1] =="flying").map( p => p.types[0] )

const normalTypeNumber = pokémon.filter( p => p.types[0] == "normal" ).length

console.log(normalTypeNumber)
